...
# Also generates docker-compose.release.yml override at repo root.
#
# Usage: ./scripts/build-release-images.sh
#
# Produces:
#   dist/.release-version                    ← version tag used (e.g. v1.2.0)
#   dist/plane-frontend-v*.tar.gz
#   dist/plane-admin-v*.tar.gz
#   dist/plane-space-v*.tar.gz
#   dist/plane-live-v*.tar.gz
#   dist/plane-backend-v*.tar.gz
#   docker-compose.release.yml               ← override file for production server
...
usage() {
  cat <<'EOF'
Usage: ./scripts/build-release-images.sh [options]
...
echo "${TAG}" > "${DIST_DIR}/.release-version"
echo "Version manifest written → ${DIST_DIR}/.release-version"
echo ""

# ── Generate docker-compose.release.yml override ─────────────────────────────────
{
  cat <<EOF
# Auto-generated by scripts/build-release-images.sh — do not edit manually.
# Version: ${TAG}
# Transfer this file to the production server alongside dist/ and scripts/deploy-release.sh
#
# Usage on server:
...
echo "Override file generated → docker-compose.release.yml"
echo ""
...
echo "     scp -r ${DIST_DIR}/ docker-compose.release.yml scripts/deploy-release.sh user@server:/path/to/plane/"
echo "  2. On server: chmod +x /path/to/plane/scripts/deploy-release.sh"
echo "  3. On server: cd /path/to/plane && ./scripts/deploy-release.sh"
echo ""
