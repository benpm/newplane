...
# Reads version from dist/.release-version, loads tar.gz archives into Docker,
# runs Django migrations (force-recreate migrator), then deploys all services.
#
# Usage: ./scripts/deploy-release.sh [dist-dir] [env-file] [base-compose]
#
# Arguments:
...
#   - dist/ folder with tar.gz archives and .release-version file
#   - docker-compose.release.yml override file in working directory
...
OVERRIDE_COMPOSE="docker-compose.release.yml"
...
VERSION_FILE="${DIST_DIR}/.release-version"
[ -f "${VERSION_FILE}" ] || {
  echo "ERROR: ${VERSION_FILE} not found."
  echo "       Run scripts/build-release-images.sh on the build machine first."
  exit 1
}
...
# --force-recreate: recreate containers even when config is unchanged, so the
# newly loaded release images replace the running ones.
