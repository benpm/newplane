...
class GlobalProjectSerializer(DynamicBaseSerializer):
    """Lean serializer for cross-workspace global project list."""
...
class WorkspaceGlobalProjectsEndpoint(BaseAPIView):
    """
    Returns global projects visible to the requesting user.

    Visibility rules:
    - Instance admin: all global projects across all workspaces.
    - Department manager: all global projects in workspaces linked to
      their managed departments and every descendant department (dept tree),
      plus any global projects they have explicitly joined.
    - Workspace admin (non-dept-manager): global projects they are a
      project member of, plus all global projects in workspaces where
      they hold the Admin role.
    - Regular member (non-dept-manager, non-workspace-admin): only
      global projects they are a project member of.
    """
    ...
    def get(self, request, slug):
        ...
        # Instance admin → all global projects
        if _is_instance_admin(request.user):
            projects = base_qs.filter(is_global=True, **archived_filter)
            return Response(GlobalProjectSerializer(projects, many=True).data, status=status.HTTP_200_OK)

        user = request.user

        # Department manager: see all global projects in managed dept-tree workspaces
        managed_dept_ids = [
        ...
        if managed_dept_ids:
            ...
            projects = base_qs.filter(is_global=True, **archived_filter).filter(
                Q(workspace_id__in=dept_ws_ids) | Q(id__in=joined_project_ids)
            )
            return Response(GlobalProjectSerializer(projects, many=True).data, status=status.HTTP_200_OK)

        # Workspace admins additionally see all global projects in their admin workspaces
