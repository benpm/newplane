...
class TestProjectFieldPermissionActivity:
    def test_patch_single_toggle_emits_one_activity(self, admin_user, workspace, project, fp_row):
        ...
        call = mock_activity.call_args

        # model_activity diffs requested_data against current_instance key by key,
        # so the new value arrives as a flat map and the old value as a JSON string.
        requested_data = call.kwargs["requested_data"]
        current_instance = json.loads(call.kwargs["current_instance"])

        assert requested_data["allow_member_modify_target_date"] is True
        assert current_instance["allow_member_modify_target_date"] is False
