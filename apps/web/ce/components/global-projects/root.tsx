...
import { GlobalProjectsService } from "@/plane-web/services/global-projects.service";
import { GlobalProjectCard } from "./project-card";

const globalProjectsService = new GlobalProjectsService();

export const GlobalProjectsRoot = function GlobalProjectsRoot() {
  ...
  const swrKey = currentWorkspace?.slug ? `GLOBAL_PROJECTS_${currentWorkspace.slug}_${showArchived}` : null;
  const {
    data: projects,
    isLoading,
    error,
  } = useSWR(swrKey, () => globalProjectsService.fetchAll(currentWorkspace!.slug, showArchived));
  ...
  const grouped = useMemo(() => {
    ...
    return filtered.reduce<Record<string, IGlobalProject[]>>((acc, project) => {
  ...
  if (error) {
    return (
      <EmptyStateDetailed
        ...
        description={t("global_projects.empty_state")}
        assetKey="project"
